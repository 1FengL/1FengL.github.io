...
In NLP, words are the main objects that we are dealing with. How should we represent words in a form that computers can store? A straightforward answer can be representing them as discrete symbols:
...
Word2vec is a framework for learning word vectors. Under this framework, we represent each word by a vector in a way the probability:
